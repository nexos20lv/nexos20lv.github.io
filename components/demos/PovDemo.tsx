...
export default function PovDemo() {
  ...
  return (
    <div className="pov-demo flex flex-col items-center w-full p-2 select-none">
      {/* Top Telemetry Header */}
      <div className="flex items-center justify-between w-full px-2.5 py-1 text-[11px] font-mono border border-ice-800/60 bg-slate-950/80 rounded-t-lg">
        <div className="flex items-center gap-2">
          <span className="inline-block w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
          <span className="text-ice-200 font-semibold tracking-wider uppercase text-[10px] sm:text-[11px]">
            POV Engine · {rpm} RPM
          </span>
          <span className="text-ice-500 text-[10px]">
            ({(rpm / 60).toFixed(1)} Hz)
          </span>
        </div>
      ...
      {/* Canvas Viewport */}
      <div className="relative flex items-center justify-center w-full bg-slate-950 border-x border-ice-800/60 py-1">
        <canvas
          ref={canvasRef}
          style={{ width: "100%", maxWidth: "420px", height: "180px" }}
          className="bg-slate-950 block"
        />

        {/* Floating status label when 1200 RPM is reached */}
        {rpm >= 1100 && (
          <div className="absolute bottom-2 left-3 bg-cyan-950/90 border border-cyan-500/50 text-cyan-300 text-[10px] font-mono px-2 py-0.5 rounded backdrop-blur pointer-events-none shadow-[0_0_10px_rgba(6,182,212,0.3)]">
      ...
      {/* Controls & Presets */}
      <div className="w-full bg-slate-950/90 border border-ice-800/60 rounded-b-lg p-2.5 space-y-2">
        {/* RPM Slider */}
        <div className="flex items-center gap-3">
          <label htmlFor={sliderId} className="text-[11px] font-mono text-ice-300 whitespace-nowrap min-w-[70px]">
            ...
            {presets.map((p) => (
              <button
                ...
                className={`text-[10px] font-mono px-2 py-1 rounded transition-all ${
                  rpm === p.value
                    ? "bg-cyan-500/25 text-cyan-200 border border-cyan-400/60 shadow-[0_0_8px_rgba(6,182,212,0.3)] font-semibold"
          ...
          <div className="flex items-center gap-1">
            ...
            {(["nexos", "radar", "telemetry"] as PatternType[]).map((pat) => (
              <button
                ...
                className={`text-[10px] font-mono px-2 py-1 rounded uppercase tracking-wider transition-all ${
                  pattern === pat
                    ? "bg-blue-600/30 text-blue-200 border border-blue-400/60 font-semibold"
                    : "bg-slate-900 text-ice-400 hover:text-ice-200 border border-slate-800"
                }`}
              >
